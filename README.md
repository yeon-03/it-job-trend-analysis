...
본 프로젝트는 **국내 IT 채용 공고 18,225건을 수집**하고 전처리 후 **9,714건을 분석**하여 기술 스택 트렌드, 직무별 채용 패턴, 기업 규모별 요구사항을 시각화하고,  
RAG 챗봇과 Fine-tuned LLM을 통해 **맞춤형 커리어 로드맵**을 생성하는 대화형 대시보드 시스템입니다.

- **기간**: 2026.05 ~ 2026.06
- **데이터**: 원티드 16,839건 · 사람인 1,375건 · 잡다임 11건 (총 수집 **18,225건** · 분석 **9,714건**)
- **분석 기간**: 2025.07 ~ 2026.03 (월별 트렌드)
...
분석 결과가 이미 포함되어 있어 **원본 데이터 없이 바로 실행** 가능합니다.
...
> |------|:-------------------:|
> | 대시보드 (차트 · 통계) | ✅ 정상 동작 |
> | RAG 챗봇 (`/api/chat`) | ❌ 벡터 DB 미포함 (119MB) |
> | 로드맵 생성 (`/api/roadmap`) | ❌ 모델 가중치 미포함 |
>
> RAG 챗봇과 로드맵 생성은 전체 파이프라인 실행 후 동작합니다.

> `bitsandbytes`는 CUDA GPU 환경에서만 정상 동작합니다.  
...
> ⚠️ 원본 수집 데이터는 용량 문제로 저장소에 포함되지 않습니다.  
> 아래 CSV 파일 4개를 직접 준비하여 **프로젝트 루트**에 배치해야 합니다.
>
...
| 백엔드 개발자 3개월 로드맵 | AI/ML 개발자 6개월 로드맵 |
|:---:|:---:|
| ![백엔드 개발자 3개월 로드맵 (대기업 목표)](outputs/roadmaps/백엔드_개발자_3개월_로드맵.png) | ![AI/ML 개발자 6개월 로드맵 (대기업 목표)](outputs/roadmaps/AI_ML_개발자_6개월_로드맵.png) |
